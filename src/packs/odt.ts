...
const PACK_ID = "anthropies.odt"
const PACK_VERSION = "0.4.0"
const contractEvidence = (): Evidence => new Evidence({ kind: "contract" })

const pathOf = (artifact: Artifact): string => artifact.name ?? "owned.odt"

export const odtPack: CapabilityPack = {
  manifest: Schema.decodeUnknownSync(CapabilityManifest)({
    id: PACK_ID,
    displayName: "ODT metadata",
    kernelApiMin: "1.0.0",
    kernelApiMax: "1.0.0",
    apiVersion: "1.0.0",
  ...
  inspect: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const scanned = inspectOdt(artifact.bytes, pathOf(artifact))
      if (!scanned.ok) {
  ...
  transform: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const cleaned = cleanOdt(artifact.bytes, pathOf(artifact))
      if (!cleaned.ok) {
        return yield* Effect.fail(
          new CapabilityFailure({
            code: "unavailable",
      ...
      const next = makeArtifact(cleaned.bytes, "odt")
