...
export const mdPack: CapabilityPack = {
  ...
  inspect: (artifact: Artifact, _context: RunContext) =>
    Effect.sync(() => {
      const scanned = inspectMdText(new TextDecoder("utf-8").decode(artifact.bytes))
      return [
        new KernelFinding({
          channel: "c2pa",
  ...
  transform: (artifact: Artifact, _context: RunContext) =>
    Effect.sync(() => {
      const cleaned = cleanMdText(new TextDecoder("utf-8").decode(artifact.bytes))
      const next = makeArtifact(cleaned.bytes, "md")
