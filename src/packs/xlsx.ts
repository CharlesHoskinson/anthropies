...
import { cleanXlsx, inspectXlsx } from "../formats/xlsx.js"

const PACK_ID = "anthropies.xlsx"
const PACK_VERSION = "0.4.0"
const contractEvidence = (): Evidence => new Evidence({ kind: "contract" })

const pathOf = (artifact: Artifact): string => artifact.name ?? "owned.xlsx"

export const xlsxPack: CapabilityPack = {
  manifest: Schema.decodeUnknownSync(CapabilityManifest)({
    id: PACK_ID,
    displayName: "XLSX metadata",
    kernelApiMin: "1.0.0",
    kernelApiMax: "1.0.0",
    apiVersion: "1.0.0",
  ...
  inspect: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const scanned = inspectXlsx(artifact.bytes, pathOf(artifact))
      if (!scanned.ok) {
  ...
  transform: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const cleaned = cleanXlsx(artifact.bytes, pathOf(artifact))
      if (!cleaned.ok) {
        return yield* Effect.fail(
          new CapabilityFailure({
            code: "unavailable",
      ...
      const next = makeArtifact(cleaned.bytes, "xlsx")
