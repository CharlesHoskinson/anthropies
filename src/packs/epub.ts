...
import { cleanEpub, inspectEpub } from "../formats/epub.js"

const PACK_ID = "anthropies.epub"
const PACK_VERSION = "0.4.0"
const contractEvidence = (): Evidence => new Evidence({ kind: "contract" })

const pathOf = (artifact: Artifact): string => artifact.name ?? "owned.epub"

const failClosed = (reason: string): CapabilityFailure => {
  const resource = /zip expansion exceeds/i.test(reason)
  return new CapabilityFailure({
    code: resource ? "resource-exceeded" : "unavailable",
    packId: PACK_ID,
    reason: resource ? "resource-exceeded" : "malformed-output",
    diagnostics: reason
  })
}

export const epubPack: CapabilityPack = {
  manifest: Schema.decodeUnknownSync(CapabilityManifest)({
    id: PACK_ID,
    displayName: "EPUB metadata",
    kernelApiMin: "1.0.0",
    kernelApiMax: "1.0.0",
    apiVersion: "1.0.0",
  ...
  inspect: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const scanned = inspectEpub(artifact.bytes, pathOf(artifact))
      if (!scanned.ok) {
        return yield* Effect.fail(failClosed(scanned.reason))
      }
      return [
        new KernelFinding({
          channel: "c2pa",
  ...
  transform: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const cleaned = cleanEpub(artifact.bytes, pathOf(artifact))
      if (!cleaned.ok) {
        return yield* Effect.fail(failClosed(cleaned.reason))
      }
      const next = makeArtifact(cleaned.bytes, "epub")
