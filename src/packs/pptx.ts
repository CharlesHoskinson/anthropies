...
import { cleanPptx, inspectPptx } from "../formats/pptx.js"

const PACK_ID = "anthropies.pptx"
const PACK_VERSION = "0.4.0"
const contractEvidence = (): Evidence => new Evidence({ kind: "contract" })

const pathOf = (artifact: Artifact): string => artifact.name ?? "owned.pptx"

export const pptxPack: CapabilityPack = {
  manifest: Schema.decodeUnknownSync(CapabilityManifest)({
    id: PACK_ID,
    displayName: "PPTX metadata",
    kernelApiMin: "1.0.0",
    kernelApiMax: "1.0.0",
    apiVersion: "1.0.0",
  ...
  inspect: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const scanned = inspectPptx(artifact.bytes, pathOf(artifact))
      if (!scanned.ok) {
  ...
  transform: (artifact: Artifact, _context: RunContext) =>
    Effect.gen(function* () {
      const cleaned = cleanPptx(artifact.bytes, pathOf(artifact))
      if (!cleaned.ok) {
        return yield* Effect.fail(
          new CapabilityFailure({
            code: "unavailable",
      ...
      const next = makeArtifact(cleaned.bytes, "pptx")
